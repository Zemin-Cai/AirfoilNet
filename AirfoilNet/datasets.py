...
class ADatasets(Dataset):
    def __init__(self, root, symmetrica=False, test=False):
        super().__init__()
        self.root = root
        self.symmetrica = symmetrica
        self.test = test

        self.data_list = glob(os.path.join(root, 'NA*/*.txt'))
...
def data_loader(root, batch_size, test):
    if not test:
        r = 1
    else:
        r = 1
    data = ADatasets(root, symmetrica=False, test=test)
    data_gener = data
    # if needed, add NACA 5 digit airfoils and generated data

    # data_five = ADatasets(r'data/NACA 5 digit airfoils', symmetrica=False, test=test)
    # data_gener = ADatasets(r'data/NACA_DATA_ddim_step50', symmetrica=False, test=test)

    split = [int(len(data) * r), len(data) - int(len(data) * r)]
    data, _ = random_split(data, split)

    # split = [int(len(data_five) * r), len(data_five) - int(len(data_five) * r)]
    # data_f, _ = random_split(data_five, split)

    # split = [int(len(data_gener) * (1-r)), len(data_gener) - int(len(data_gener) * (1-r))]
    # data_gener, _ = random_split(data_gener, split)

    # data_ = data + data_gener + data_f
    data_ = data

    print('Data Nums:', len(data_))
    print('Generated data', len(data_gener))
    if test:
        return DataLoader(data, batch_size=batch_size, pin_memory=4, shuffle=True)

    return DataLoader(data_, batch_size=batch_size, pin_memory=4, shuffle=True)
...
if __name__ == '__main__':
    d = ADatasets(root=r'data/NACA 5 digit airfoils', symmetrica=False, test=False)

    for i in DataLoader(d, batch_size=32):
